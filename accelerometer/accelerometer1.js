...
var bathroomQuotes = ['too busy', 'no go', 'try another time', 'bathroom busy now', 'maybe not right now']
...
accel.on('data', function (xyz) {
  ...
  if (!hasChanged(xyz) && movement && !doorClosed) {
    if(queue.length) {
      if(new Date().getTime() - queue[0] > 4000) {
        queue.shift()
      } else {
  ...
  if(queue.length === stallNum) {
    ...
    twitterRestClient.statusesUpdate(
      {status: 'Too many people!'},//'bathroom busy ' + new Date().toString()},
      function (err, data) {
        if (err) {
          console.error(err);
        } else {
